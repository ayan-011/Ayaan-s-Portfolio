...
function Noise({
  ...
  useEffect(() => {
    ...
    const patternPixelDataLength =
      patternSize * patternSize * 4;
    ...
    const updatePattern = () => {
      for (
        let i = 0;
        i < patternPixelDataLength;
        i += 4
      ) {
        const value =
          Math.random() * 255 * intensity;
    ...
    const drawGrain = () => {
      ...
      const safePatternScaleX = Math.max(
        0.001,
        patternScaleX
      );

      const safePatternScaleY = Math.max(
        0.001,
        patternScaleY
      );
      ...
      const fillPattern = ctx.createPattern(
        patternCanvas,
        "repeat"
      );
...
function GradientBackground({
  ...
  noisePatternScaleX = 0.5,
  noisePatternScaleY = 0.5,
  ...
  const generateGradient = () => {
    ...
    if (
      ...
      const angle =
        angleMap[gradientOrigin] ||
        angleMap["bottom-middle"];
